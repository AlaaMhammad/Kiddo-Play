<x-admin title="Add Parent-Child Relation">

    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="fw-bold mb-0">Add Relation</h4>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.parent-children.store') }}" method="POST">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <select name="kid_id" id="kid_id" class="form-select @error('kid_id') is-invalid @enderror">
                        ...
                        @foreach($kids as $id => $display_name)
                            <option value="{{ $id }}" {{ old('kid_id') == $id ? 'selected' : '' }}>{{ $display_name }}
                            </option>
                        @endforeach
                    </select>
                    @error('kid_id')
