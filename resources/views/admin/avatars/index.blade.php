                    ...
                    <tbody>
                        @foreach ($avatars as $avatar)
                            <tr>
                                ...
                                <td><img src="{{ asset('storage/' . $avatar->image_url) }}" width="60" height="60"
                                        class="rounded-circle"></td>
                                <td>{{ $avatar->name }}</td>
                                <td>{{ $avatar->cost_points }}</td>
                                <td>{!! $avatar->is_active ? '<span class="badge bg-label-success">Active</span>' : '<span class="badge bg-label-danger">Inactive</span>' !!}
                                </td>
                                <td>
                                    <a href="{{ route('admin.avatars.edit', $avatar) }}" class="btn btn-sm btn-warning"><i
